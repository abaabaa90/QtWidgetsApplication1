...
#include "vtkPoints.h"
#include <vtkStringFormatter>
#include <vtkStringScanner>
...
//static const bool isDelimiter[256] = {
//    // ASCII 0-255，只初始化特定字符
//    [' '] = true,  // 空格
//    ['\n'] = true, // 换行
//    ['\r'] = true, // 回车
//    ['\t'] = true, // 制表符
...
public:
    ...
    {
        // 如果有一个标记被备份（即前一次读取时多读了一个标记），则直接返回这个备份标记
        // 这是实现单标记前瞻（lookahead）机制的关键
        if (!this->TokenBackup.empty())
        {
            std::string retval = this->TokenBackup; // 获取备份的标记
            this->TokenBackup = ""; // 清空备份标记
            return retval; // 返回备份标记
        }
        ...
        // 重置行结束标志和字符串标记标志
        this->NextCharEOL = false;  // 标记当前是否遇到行结束符
        this->TokenIsString = false; // 标记当前提取的标记是否为带引号的字符串

        std::string retval; // 用于存储最终返回的标记字符串

        // 如果下一个字符无效（即尚未从文件流中读取字符），则读取一个字符
        if (!this->NextCharValid)
        {
            this->TheNextChar = this->ASCIIStream.get(); // 从文件流中读取一个字符
            this->NextCharValid = true; // 标记字符已读取，现在有效

            // 检查读取后是否到达文件末尾
            if (!this->ASCIIStream)
            {
                this->NextCharEOF = true; // 设置EOF标志
            }
        }
        ...
        if (!this->NextCharEOF && this->TheNextChar == '#')
        {
            ...
            {
                this->TheNextChar = this->ASCIIStream.get(); // 读取下一个字符
                // 如果遇到换行符，设置行结束标志
                if (this->TheNextChar == '\n' || this->TheNextChar == '\r')
                {
                ...
                this->TheNextChar == ')' || this->TheNextChar == ','))
        {
            ...
            if (this->NextCharEOL)
            {
                return this->GetNextToken(); // 递归调用以获取下一个标记
            }
        }
        ...
        if (this->TheNextChar == '\"')
        {
            this->TokenIsString = true; // 标记当前提取的标记为字符串
            // 读取双引号后的第一个字符
            this->TheNextChar = this->ASCIIStream.get();
            // 检查是否到达文件末尾
            if (!this->ASCIIStream)
            ...
            while (!this->NextCharEOF && this->TheNextChar != '\"')
            {
                retval += this->TheNextChar; // 将字符添加到返回字符串
                this->TheNextChar = this->ASCIIStream.get(); // 读取下一个字符
        ...
        else
        {
                    ...
                    this->TheNextChar != ')' && this->TheNextChar != ','))
            {
                ...
                {
                    this->TheNextChar += (int('A') - int('a')); // 转换为大写
                }
